...
const Edit = ({ articles }) => {
    ...
    useEffect(() => {
        if (router.query.id) {
            const article = articles.find(art => art._id == router.query.id)
            console.log(article)
            setForm({
                title: article.title,
                slug: article.slug
            ...
            setImages(article.coverImg)
        }
    }, [router.query.id])

    useEffect(() => {
        if (quill) {
            quill.root.innerHTML = excerpt
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        const sendData = { id, form, text, excerpt, images, stage: e.target.name == 'save_btn' ? 'Draft' : 'Published' };

        const t = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/updateblog`, {
            method: 'POST', // or 'PUT'
            headers: {
